...
//
//              dataList.add(Data(i, series?.get(i-1)?.title!!, series?.get(i-1)?.shortDescription!!,series?.get(i-1)?.thumbnail))
//                dataList2.add(OtherData(0, movies?.get(i-1)?.title!!, movies?.get(i-1)?.shortDescription!!,
...
//    fun getData1(): List<OtherData> {
//        val dataList = mutableListOf<OtherData>()
//        for (j in 0..20){
//            for (i in 1..movies?.size!!) {
//                dataList.add(OtherData(0, movies?.get(i-1)?.title!!, movies?.get(i-1)?.shortDescription!!,
//                   movies?.get(i-1)?.thumbnail))
//
//                }
//            }
